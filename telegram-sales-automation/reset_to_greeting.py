#!/usr/bin/env python3
"""
reset_to_greeting.py — сброс данных до состояния «после приветствия» (до wf3)

Что делает:
  1. Google Sheets: очищает Статус, Запрос, Дата контакта (Аккаунт НЕ трогает)

Использование:
  python3 reset_to_greeting.py                     # сброс всего
  python3 reset_to_greeting.py --account anasty_4  # только один аккаунт
"""
...
def reset_sheets(account_id=None):
    webhook_path = f"reset-greeting-{uuid.uuid4().hex[:8]}"

    # Фильтр: строки с wf3-данными; Аккаунт не трогаем
    if account_id:
        filter_expr = (
        ...
        filter_expr = (
            ...
            "  i.json['Статус'] || i.json['Запрос'] || i.json['Дата контакта']\n"
            ");\n"
            "return items.filter(i => String(i.json.User_id || '').trim() !== '');"
        )

    workflow = {
        "name": "_tmp_reset_to_greeting",
        "nodes": [
            {
                "id": "rn1",
                "name": "Webhook",
                ...
                "parameters": {
                    ...
                    "columns": {
                        ...
                        "value": {
                            ...
                            "Запрос":        "",
                            "Дата контакта": ""
                            # Аккаунт — намеренно не сбрасывается
                        },
        ...
        "connections": {
            ...
            "Filter":     {"main": [[{"node": "Clear WF3 Fields", "type": "main", "index": 0}]]},
        },
...
def main():
    parser = argparse.ArgumentParser(
        description="Сброс данных до состояния «после приветствия» (до wf3)"
    )
    parser.add_argument(
        "--account",
        metavar="ACCOUNT_ID",
    ...
    print(f"\nСброс до «после приветствия»: {label}\n{'─'*50}")

    print("[Google Sheets] Очищаем Статус, Запрос, Дата контакта (Аккаунт сохраняется)...")
    try:
        reset_sheets(args.account)
